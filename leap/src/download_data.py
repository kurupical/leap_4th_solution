...
def make_webdataset_month(exp_name: str, month_dir: str, data_utils: DataUtils) -> None:
    # clear tmp dir
    ...
    shard_size = int(1024 * 1000**2)  # shard_size_m MB each
    ...
    for input_path in tqdm(input_paths):
        ...
        os.makedirs("input/climsim_low-res/npy/", exist_ok=True)
        np.save(
            "input/climsim_low-res/npy/" + input_path.stem + "_input.npy", input_data
        )
        np.save(
            "input/climsim_low-res/npy/" + input_path.stem + "_output.npy", output_data
        )


def make_webdataset(exp_name) -> None:
    grid_path = "input/climsim_low-res/ClimSim_low-res_grid-info.nc"
    norm_path = "input/climsim_low-res/preprocessing/normalizations/"
    ...
    month_dirs = [
        f"train/000{y}-{str(m).zfill(2)}" for y in range(1, 9) for m in range(1, 13)
    ] + ["train/0009-01"]
    ...
    for month_dir in month_dirs:
        ...
        make_webdataset_month(exp_name, month_dir, data_utils)


def main() -> None:
    exp_name = f"{Path(sys.argv[0]).parent.name}/"

    print(f"exp_name: {exp_name}")
    output_path = Path("preprocess") / exp_name
    print(f"ouput_path: {output_path}")
    os.makedirs(output_path, exist_ok=True)

    make_webdataset(exp_name)
